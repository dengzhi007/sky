...
func (this *Backend) sending(point *Backpoint) {
	...
	this.logger.Info("Backend%d back point %s start to work", this.index, point.name)
	...
	for this.running {
		...
		this.logger.Info("Backend%d send data out to %s success, remaining %d packets in sending channel", this.index, point.name, len(point.sendingChan))
	}
...
func (this *Backend) send(bytes []byte, point *Backpoint) error {
	...
	this.logger.Info("Backend%d write to %s success, duration %v, IP:%s, Length:%d, Content:%s", this.index, point.name, time.Now().Sub(start), point.backAddrs[point.currentBackIdx], sentNum, string(bytes))
